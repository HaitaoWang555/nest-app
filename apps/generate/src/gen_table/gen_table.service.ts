...
export class GenTableService {
  ...
  async generate(id: number, templeteFiles?: string[]) {
    ...
    const beTempletePathStr = __dirname + '/assets/dot/be';
    const feTempletePathStr = __dirname + '/assets/dot/fe';
    const generateFilePathPackage = data.package ? data.package + '/' + data.tableName : data.tableName;
    const fileSuffix = '.dot';

    // 后端入口文件生成
    if (data.generateBePtah && [0, 2].includes(data.generateType)) {
      const generateBeFilePathPrefix = data.generateBePtah + '/src/' + generateFilePathPackage;
      const beFilesPath = await glob(beTempletePathStr + '/**/*' + fileSuffix);
      this.writeFileByTemplete(
        beFilesPath,
        generateBeFilePathPrefix,
        beTempletePathStr,
        fileSuffix,
        data,
        templeteFiles,
    ...
    // 前端入口文件生成
    if (data.generateFePtah && [0, 1].includes(data.generateType)) {
      const generateFeFilePathPrefix = data.generateFePtah + '/src/views/' + generateFilePathPackage;
      const feFilesPath = await glob(feTempletePathStr + '/**/*' + fileSuffix);
      this.writeFileByTemplete(
        feFilesPath,
        generateFeFilePathPrefix,
        beTempletePathStr,
        fileSuffix,
        data,
        templeteFiles,
  ...
  private generateBeInletFile(data: GenTable, generateFilePathPackage: string) {
    const modulesFile = readFileSync(data.generateBePtah + '/src/modules.ts');
    const str = `export { ${camel(data.tableName, true)}Module } from './${generateFilePathPackage}/${
      data.tableName
    }.module';`;
    if (!modulesFile.includes(str)) {
      // 不重复生成
      writeFileSync(data.generateBePtah + '/src/modules.ts', modulesFile.toString() + str + EOL);
    }
  }

  private generateFeInletFile(data: GenTable) {
    const baseFeRouterPath = data.generateFePtah + '/src/routes/modules';
    if (!data.package) {
      const routerFileTemplete = readFileSync(__dirname + '/assets/dot/fe/router.dot');
      ...
      // 有 data.package 情况
      const routerFileTemplete = readFileSync(__dirname + '/assets/dot/fe/package-router.dot');
  ...
  private writeFileByTemplete(
    ...
    for (let index = 0; index < filesPath.length; index++) {
      ...
      const generateFileSuffix = str.includes('list') ? '.vue' : '.ts';
